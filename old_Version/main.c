...
void main(void) {
     ...
     ****************************************************************/
         ...
         * CONSEQ_2单通道重复转换模式
         */
        ADC10CTL0 |= SREF_1 | ADC10SHT_2 | REFON | REF2_5V ;//这里的设置是
        ADC10CTL1 |= INCH_0  | ADC10DIV_1 | ADC10SSEL_2 | CONSEQ_0 ;//这里使用的是主时钟2分频8MHZ，采样保持时间为1/8MHZ * 16 =2us 满足最少3.9us的要求  首先采集的是滤波网络的输出
        //这里设置的是P1.0乘法器输出
        //P1.5为滤波网络的输出
    /*****************************************************************
     * init TA
...
void freqChange(){
    ...
    TA0CCR0=0;
    TA1CCR0=0;
    ADC10CTL0&=~(ADC10IE|ENC|ADC10SC|ADC10ON);
    phaResult=(maxPhaADResult/1024.0)*2.5;
    ampResult=(maxAmpADResult/1024.0)*2.5;
    float temp=0.0;
    if(Freq_past!=Freq_now){
    ...
    if(state==0xA2){
         ...
         snprintf(sendBuff,sendBuffLength,"add 1,0,%d",(int)(ampResult*100.0));
         UART_OnTX(sendBuff,11);
         snprintf(sendBuff,sendBuffLength,"add 1,1,%d",(int)(phaResult*100.0));
         UART_OnTX(sendBuff,11);
         snprintf(sendBuff,sendBuffLength,"n0.val=%d",Freq_now);
         UART_OnTX(sendBuff,11);
    }else if(state==0xB0){
    }else if(state==0xA1){
        snprintf(sendBuff,sendBuffLength,"amp.txt=\"%d\"",(int)(ampResult*100.0));
    ...
    //重新配置ADC
    ADC10CTL1&=~INCH_15;
    ADC10CTL1|=INCH_0;//改为去采集滤波网络的输出
    ADC10CTL0|=ADC10ON | ENC |ADC10IE|ADC10SC; //重新开启ADC10的中断并开始转换
    //==========================================
    //在函数调用结尾时重新开放定时器
...
//            float temp=PointMax/((1/Freq_now)/0.00001);
//            int setSinINTInterval=(int)temp;
//            int setSinDECInterval=(int)(((float)temp-(int)temp)*1000.0);
//            sinIntInterval=setSinINTInterval;
//            sinDecSum=setSinDECInterval;
            TA0CCR0=TA0CCR0_VAL;
            TA0CTL|=MC_1;
...
__interrupt void ADC10_ISR_HOOK(void)
{
    ...
    _disable_interrupts();
    switch(ADC_ConvertFlag){
    case 0x00:
        ampADResult=ADC10MEM;
        if(ampADResult>maxAmpADResult) maxAmpADResult=ampADResult;
        ADC10CTL1&=~INCH_15;
        ADC10CTL1|=INCH_0;//改为P1.0端口接收乘法器的输出
        ADC_ConvertFlag=0x01;
        break;
    case 0x01:
        phaADResult=ADC10MEM;
        if(phaADResult>maxPhaADResult) maxPhaADResult=phaADResult;
        ADC10CTL1&=~INCH_15;
        ADC10CTL1|=INCH_5;//改为P1.5端口接收信号器的输出
        ADC_ConvertFlag=0;
        break;
    default:
        break;
    }
    ADC10CTL0 |=  ADC10ON|ENC;//不停的启动AD转换
    _enable_interrupts();
...
__interrupt void TIMER1_A1_ISR_HOOK(void)
{
    ...
    switch(__even_in_range(TA1IV,TA1IV_TAIFG)){
        ...
        case TA1IV_TAIFG : freqChange();break;
        default : break;
        }
    //_nop();
    _enable_interrupts();
